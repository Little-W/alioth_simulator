 ...
 */
void rt_rv32_system_irq_handler(rt_uint32_t mcause, rt_uint32_t mepc, rt_uint32_t sp)
{
    rt_uint32_t irq_id = (mcause & 0x1F);
    // Save the stack frame pointer
    s_stack_frame = (volatile rt_hw_stack_frame_t *)(uintptr_t)sp;
...
{
    plic_disable_irq((uint8_t)vector);
}

void rt_hw_interrupt_umask(int vector)
{
    plic_enable_irq((uint8_t)vector);
}
