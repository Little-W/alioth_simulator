...
def get_env_root():
    ...
    if env_root is None:
        # Try to determine env_root based on script location
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # Go up from tools/scripts to env root
        env_root = os.path.dirname(os.path.dirname(script_dir))
        
        # If that doesn't work, fall back to old behavior
        if not os.path.exists(env_root) or not os.path.isdir(env_root):
